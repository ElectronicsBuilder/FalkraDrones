...
int32_t VL53L5CX_ConfigProfile(VL53L5CX_Object_t *pObj, VL53L5CX_ProfileConfig_t *pConfig)
{
  ...
  else
  {
    ...
    pObj->CachedResolution = resolution;
...
int32_t VL53L5CX_ConfigIT(VL53L5CX_Object_t *pObj, VL53L5CX_ITConfig_t *pITConfig)
{
  ...
  {
    if (pObj->CachedResolution != 0U)
    {
      res = pObj->CachedResolution;
    }
    else
    {
...
static int32_t vl53l5cx_get_result(VL53L5CX_Object_t *pObj, VL53L5CX_Result_t *pResult)
{
  ...
  else
  {
    if (pObj->CachedResolution != 0U)
    {
      resolution = pObj->CachedResolution;
    }
    ...
    if (ret == VL53L5CX_OK)
    {
      ...
      for (i = 0; i < resolution; i++)
      {
        ...
        uint8_t target_count = data.nb_target_detected[i];
#else
        uint8_t target_count = VL53L5CX_NB_TARGET_PER_ZONE;
#endif

        pResult->ZoneResult[i].NumberOfTargets = target_count;
