...
#define BQ72441_I2C_TIMEOUT 100
...
public:
	...
	*/
	float temperature(temp_measure type = BATTERY);
	...
	*/
	...
	uint16_t readControlStatus();

	uint16_t getStatusFlags();  // Reads and returns CONTROL_STATUS flags
	...
	*/	
	...
	bool setFCThresholds(int8_t set, int8_t clear);
	uint8_t FCSetThreshold(void);
	uint8_t FCClearThreshold(void);
	
	bool setTCAThresholds(int8_t set, int8_t clear);
	uint8_t TCASetThreshold(void);
	uint8_t TCAClearThreshold(void);


private:
    I2C_HandleTypeDef* _i2c;
   static constexpr uint8_t _addr = 0x55 << 1;
